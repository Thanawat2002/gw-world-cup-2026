...
export default function Home() {
  ...
  const handleEliminate = (id: number, roundId: string) => {
    ...
    if (p) {
      ...
      const suffix = isChamp ? `${p.name} แชมป์โลก!` : order <= 5 ? `${p.name} ต้องเลี้ยงเหล้า!` : `${p.name} รอดแล้ว!`;
      showToast(`${p.team} ${isChamp ? 'CHAMPION' : 'TERMINATED'} (${round?.label}) — ${suffix}`);
    }
  };
    ...
    <>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <header style={{ padding: '48px 20px 20px', textAlign: 'center' }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 5, textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 10, opacity: 0.8 }}>
            FIFA World Cup 2026 — Gentlewoman Edition
          </div>
          <h1 className="font-bebas" style={{ fontSize: 'clamp(44px, 11vw, 104px)', lineHeight: 0.88, letterSpacing: 3, color: 'var(--text)' }}>
            GW World Cup{' '}
            <em style={{ fontStyle: 'normal', color: 'var(--gold)', textShadow: '0 0 40px var(--gold-glow), 0 0 80px rgba(245,158,11,0.12)' }}>
              2026
            </em>
          </h1>
          <div className="font-bebas" style={{ fontSize: 'clamp(14px, 3.5vw, 24px)', letterSpacing: 8, color: 'var(--red)', marginTop: 6, textShadow: '0 0 20px var(--red-glow)', animation: 'pulse-red 3s ease-in-out infinite' }}>
            TERMINATOR
          </div>
        </header>

        {/* Stats bar */}
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap', margin: '0 auto', padding: '0 20px' }}>
        ...
        {showDebtAlert && (
          <div className="alert alert-debt" style={{ padding: '14px 18px' }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#F59E0B" strokeWidth="2.5" strokeLinecap="round" style={{ flexShrink: 0, marginTop: 2 }}>
              <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" />
              <path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" />
            </svg>
            <div className="alert-body">
              {debtors.map(p => <strong key={p.id}>{p.name}</strong>).reduce((acc: React.ReactNode[], el, i) => i === 0 ? [el] : [...acc, ', ', el], [])}{' '}
              ต้องเลี้ยงเหล้า <strong>{deepestEliminated?.name} ({deepestEliminated?.team})</strong> ผู้เข้ารอบลึกสุด
            </div>
          </div>
        )}
        ...
        {showWinBanner && deepestEliminated && (
          <div className="alert alert-win" style={{ padding: '20px 24px' }}>
            ...
            <div className="alert-body">
              ...
              {getRound(deepestEliminated.round) && ` (${getRound(deepestEliminated.round)?.label})`}<br />
              {debtors.length > 0 && `${debtors.map(p => p.name).join(', ')} ต้องเลี้ยงเหล้า!`}
            </div>
          </div>
        )}
        ...
        >
          {state.players.map(p => (
            <PlayerCard
              ...
              onEliminate={id => setModalPlayer(state.players.find(pl => pl.id === id) ?? null)}
        ...
        {/* Footer */}
        <footer style={{ textAlign: 'center', padding: '56px 20px 32px', color: 'var(--text-dim)', fontSize: 12 }}>
      ...
      {/* Toast */}
      {toastMsg && (
        <div key={toastKey} className="toast">{toastMsg}</div>
      )}
